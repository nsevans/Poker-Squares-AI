...
public class PokerSquares {
	...
	public static double[] playTournament(
		...
		for (PokerSquaresPointSystem system : systems) { // for each point system
			...
			for (int i = 0; i < players.size(); i++) { // for each player
				...
				int[] scores = new PokerSquares(player, system).playSequence(gamesPerSystem, startSeed, true);
	...
	private static void SingleGameDemo(PokerSquaresPointSystem system, boolean verbose)
	{
		// Demonstration of single game play (30 seconds)
		System.out.println("Demonstration of single game play (30 seconds)");
		System.out.println("Single game demo:");
		System.out.println(system);
		new PokerSquares(new MTMCTSPlayer(verbose), system).play();
	}

	private static void BatchGameDemo(PokerSquaresPointSystem system, boolean verbose)
	{
		// Demonstration of batch game play (30 seconds per game)
		System.out.println("Demonstration of batch game play (30 seconds per game)");
		for(int i=0; i<5; i++)
		{
			long startTime = System.currentTimeMillis();
			System.out.println("\n\nBatch game demo:");
			System.out.println(system);
			PokerSquaresPlayer player = new MTMCTSPlayer(verbose);
			System.out.println(player.getName());
			new PokerSquares(player, system).playSequence(5, 0, false);
			System.out.println("Time Ellapsed: "+(System.currentTimeMillis()-startTime)+"ms\n");
		}
	}

	private static void TournamentDemo(PokerSquaresPointSystem system, boolean verbose)
	{
		...
		System.out.println("Demonstration of tournament evaluation (2 players, 3 point systems, 10 x 30s games for each of the 2*3=6 player-system pairs)");
		System.out.println("\n\nTournament evaluation demo:");

		ArrayList<PokerSquaresPlayer> players = new ArrayList<PokerSquaresPlayer>();
		players.add(new RandomPlayer());
		players.add(new MTMCTSPlayer(verbose));
	...
	public static void main(String[] args) {
		...
		System.out.println("(a) Single Game Demo");
		System.out.println("(b) Batch Game Demo");
		System.out.println("(c) Tournament Demo");
		System.out.println("* Use upper case letters to run the demo but with verbosity enabled");
		...
		System.out.println("("+startDate.toString()+") Starting simulation...");
		
		if (Character.isUpperCase(choice))
			System.out.println("Playing with verbose logs enabled.");

		switch(choice)
		{
			case 'a': 
				SingleGameDemo(system, false);
				break;
			case 'A':
				SingleGameDemo(system, true);
				break;
			case 'b': 
				BatchGameDemo(system, false);
				break;
			case 'B': 
				BatchGameDemo(system, true);
				break;
			case 'c':
				TournamentDemo(system, false);
				break;
			case 'C':
				TournamentDemo(system, true);
				break;
			default:
